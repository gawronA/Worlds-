...
public static class Noise
{
	...
	private static float[] gradients1D = { 1.0f, -1.0f };
	private const int gradientsMask1D = 1;

	private static Vector2[] gradients2D = {    new Vector2(1.0f, 0.0f),
	...
	private static float sqrt2 = Mathf.Sqrt(2.0f);

	private static Vector3[] gradients3D = {    new Vector3(1.0f, 1.0f, 0.0f),
	...
	//private static Vector3[] gradients3D = {	new Vector3(1.0f)
	//										}
	...
	public static float Perlin1D(Vector3 point, float frequency)
	{
		...
		float t = Smooth(t0);
		float sample = Mathf.Lerp(v0, v1, t) * 2.0f;
	...
	public static float Perlin2D(Vector3 point, float frequency)
	{
		...
		float tx = Smooth(tx0);
		float ty = Smooth(ty0);
		float sample =  Mathf.Lerp(	Mathf.Lerp(v00, v10, tx),
	...
	public static float Perlin3D(Vector3 point, float frequency)
	{
		...
		float ty = Smooth(ty0);
		float tz = Smooth(tz0);
		float sample = Mathf.Lerp(	Mathf.Lerp(Mathf.Lerp(v000, v100, tx), Mathf.Lerp(v010, v110, tx), ty),
